...
/**
 * Ink/neutral palette for dynamic inline styles. These reference the CSS token
 * variables (rather than literal hex) so inline styles flip with the theme just
 * like the Tailwind utilities do. `MUTED` keeps the original `faint` token value.
 */
export const INK = "var(--color-ink)";
export const MUTED = "var(--color-faint)";
...
export const SOFT: Record<ItemKind, string> = {
  attraction: "var(--color-attraction-soft)",
  meal: "var(--color-meal-soft)",
  stay: "var(--color-stay-soft)",
  transport: "var(--color-transport-soft)",
  other: "var(--color-other-soft)",
};
...
export const CONTACT_COLOR: Record<ContactKind, string> = {
  ...
  family: "var(--color-meal)",
  hotel: "var(--color-stay)",
  embassy: "var(--color-transport)",
  other: "var(--color-muted)",
};

/**
 * Resolve a contact's dot colour: admin override first, then the kind default.
 * A user-chosen `c.color` is a raw hex stored on the trip — it can't be themed,
 * so it stays literal in both light and dark.
 */
export const contactColor = (c: Contact): string =>
  c.color ?? CONTACT_COLOR[c.kind] ?? "var(--color-muted)";
...
export const ui = {
  ...
  chevBtn:
    "flex h-[50px] w-[50px] shrink-0 cursor-pointer items-center justify-center rounded-[16px] border border-border bg-surface shadow-soft",
  btnPrimary:
    "h-[56px] w-full cursor-pointer rounded-[16px] border-none bg-accent text-[18px] font-bold text-white shadow-accent",
  btnGhost:
    "h-[54px] w-full cursor-pointer rounded-[16px] border border-border-strong bg-surface text-[16px] font-bold text-ink",
  input:
    "h-[54px] w-full rounded-md border border-border-strong bg-surface px-4 text-[17px] text-ink outline-none",
  tabbar:
    "flex shrink-0 border-t border-border bg-app-bg/95 pt-[6px] pb-[max(env(safe-area-inset-bottom),8px)]",
} as const;
